...
## reading in tree
tree <- read.tree("PB1_new_tree.contree") # created with iqtree
...
tree <- root(tree, node = 136, edgelabel = TRUE)
#### converting the bootstrap value outside ggtree
q <- ggtree(tree) + xlim(NA, 7.5) + expand_limits(y = 70) + coord_cartesian(clip="off")# + geom_text2(aes(label=node), hjust=-.3, size=2)  
#q <- flip(q, 330, 493)
#q <- rotate(q, 463)
...
q <- q %<+% dd +
  ...
  scale_color_manual(values = c("Anopheles" = "#E31A1C",  "Aedes" = "#1F78B4"), na.translate=F, name = "", labels = c("Anopheles",  "Aedes")) +
