...
# txt file with reference accession numbers and scientific names 

## reading in tree
tree <- read.tree("Flavi_new_tree.contree") # created with iqtree
## replacing the label descriptions
replace_words <- read.csv("Flavi_references.txt", sep = "\t", header = FALSE, col.names = c("Accession", "Virus"))
...
tree <- root(tree, node = 470, edgelabel = TRUE)
...
tipcategories = read.csv("Flavi_genomes.txt", 
                         sep = "\t",
...
q <- q %<+% dd +
  ...
  scale_color_manual(values = c("Anopheles" = "#E31A1C", "Aedes aegypti" = "orange", "Aedes albopictus" = "#1F78B4"), na.translate=F, name = "", labels = c("Anopheles", "Aedes aegypti", "Aedes albopictus")) +
